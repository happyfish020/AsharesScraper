...
def is_service_running() -> bool:
    ...
    Returns False if stopped, paused, starting, stopping, or doesn't exist.
    """
    service_name ="WireGuardTunnel$cn"
    try:
        status = win32serviceutil.QueryServiceStatus(service_name)
        # status is a tuple → index 1 is the current state
        return status[1] == win32service.SERVICE_RUNNING
    except Exception:  # pywin32 raises pywintypes.error if service doesn't exist or access denied
        return False