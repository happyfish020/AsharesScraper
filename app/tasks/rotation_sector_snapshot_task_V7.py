...
class SectorRotationSnapshotTask:
    ...
    def run(self, ctx) -> None:
        ...
        refresh_mode = str(os.getenv("ROTATION_REFRESH_MODE", "base_chain")).strip().lower()
        if refresh_mode not in {"base_chain", "sp"}:
            raise RuntimeError(
                "[rotation_snapshot] invalid ROTATION_REFRESH_MODE=%r. Expected 'base_chain' or 'sp'."
                % refresh_mode
            )
        ...
        try:
            ...
            with self.engine.begin() as conn:
                ...
                if refresh_mode == "base_chain":
                    # The historical V7/V8 deployments do not have cn_sector_rotation_transition_v.
                    # Use the built-in base-table chain as the normal path, not as a warning fallback.
