...
@dataclass
class StateStore:
    # Accept Path or str for backward/forward compatibility.
    scanned_path: Path | str
    failed_path: Path | str
    log: object

    def __post_init__(self) -> None:
        # Normalize to Path so callers can pass either Path or str.
        self.scanned_path = Path(self.scanned_path)
        self.failed_path = Path(self.failed_path)
