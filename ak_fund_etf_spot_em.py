...
def load_fund_etf_spot_em_intraday():
    today = date.today()
    print("=== Checking/Creating spot table ===")
    ...
    try:
        ...
        if df.empty:
            print("No data from ak.fund_etf_spot_em()")
            return
        ...
        print(f"Inserted/updated {len(records)} spot records for {today}.")

    except Exception as e:
        print(f"Spot load failed: {e}")
...
def load_fund_etf_hist_em(start_date: str, end_date: str, period: str = "daily", adjust: str = ""):
    ...
    """
    print(f"=== Historical load started: {start_date} → {end_date} | period={period} | adjust='{adjust}' ===")
    ...
    if df_codes.empty:
        print("No ETF codes in spot table. Run load_fund_etf_spot_em() first.")
        return

    codes = df_codes['code'].sort_values().tolist()
    print(f"Processing {len(codes)} symbols.")

    total = 0
    failed = []

    for i, symbol in enumerate(codes, 1):
        print(f"[{i:4d}/{len(codes)}] {symbol} ... ", end="", flush=True)
        try:
    ...
    if failed:
        print(f"Failed symbols: {len(failed)} | {', '.join(failed[:8])}{'...' if len(failed)>8 else ''}")
...
def load_spot_as_hist_today():
    ...
    try:
        df = ak.fund_etf_spot_em()
        if df.empty:
            print("ak.fund_etf_spot_em() 返回空数据")
            return

        print(f"获取到 {len(df)} 条当天 spot 数据")
        ...
        # 固定字段
        df['ADJUST_TYPE'] = 'SPOT_TODAY'   # 区分这是从 spot 补的当天记录
        ...
        with engine.connect() as conn:
            ...
            existing_sql = text(f"""
                ...
                WHERE DATA_DATE = :dt AND ADJUST_TYPE = :adj
            """)
        ...
        print(f"成功插入 {len(missing_records)} 条当天 spot 作为历史记录（{today_str}）")
