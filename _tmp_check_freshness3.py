"""
Check freshness of V8 tables - fixed version.
"""
...
tables_config = [
    ...
    ("cn_stock_monthly_basic", "end_date"),  # try end_date instead of month
    ("cn_stock_fina_indicator", "end_date"),
    ("cn_stock_income", "end_date"),
    ("cn_stock_balancesheet", "end_date"),
    ("cn_stock_cashflow", "end_date"),
    ("cn_board_industry_member_hist", "end_date"),  # try end_date
...
for tbl, date_col in tables_config:
    try:
        ...
        if date_col is None:
            results.append((tbl, "DIM", total, "N/A", "N/A", None, []))
            continue
...
for item in results:
    tbl, typ, total, min_d, max_d, lag, monthly = item
    
    if typ == "ERR":
        status = "ERROR"
        err_list.append((tbl, min_d))  # min_d holds error msg
    elif lag is None:
        status = "DIM"
    elif lag <= 3:
        status = "OK"
...
print(f"\n✅ FRESH tables ({len(ok_list)}): all daily tables up to {TODAY}")
...
if err_list:
    ...
    for tbl, err_msg in err_list:
        print(f"   ✗ {tbl:<50} error={err_msg[:80]}")
