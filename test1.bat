...
rem python scripts/build_local_industry_proxy_daily.py --start 20260429 --end 20260521

rem 步骤 2：补 derived foundation（角色映射等）
cd /d d:\LHJ\PythonWS\MarketScraper\AsharesScraperV2
set V8_DAILY_AUTO_REPAIR=0
python runner.py --flag tu --tasks v8_daily_derived_foundation --asof 20260521 --days 29

rem 这会覆盖从 2026-04-23 到今天的 foundation 层（cn_ga_stock_role_map_daily 等）。

rem 步骤 3：补 derived mainline（主链）
cd /d d:\LHJ\PythonWS\MarketScraper\AsharesScraperV2
set V8_DAILY_AUTO_REPAIR=0
python runner.py --flag tu --tasks v8_daily_derived_mainline --asof 20260521 --days 29

rem 这会覆盖从 2026-04-23 到今天的所有 mainline 表（strength / radar / pulse / lifecycle）。

rem 步骤 4：补 derived alpha（统一 alpha 分）
cd /d d:\LHJ\PythonWS\MarketScraper\AsharesScraperV2
set V8_DAILY_AUTO_REPAIR=0
python runner.py --flag tu --tasks v8_daily_derived_alpha --asof 20260521 --days 29

cd /d d:\LHJ\PythonWS\MarketScraper\AsharesScraperV2
python runner.py --flag tu --tasks v8_stock_fundamental_refresh --asof 20260521


rem 或者一步到位：直接跑完整 daily 链（推荐，因为有自愈逻辑）
cd /d d:\LHJ\PythonWS\MarketScraper\AsharesScraperV2
